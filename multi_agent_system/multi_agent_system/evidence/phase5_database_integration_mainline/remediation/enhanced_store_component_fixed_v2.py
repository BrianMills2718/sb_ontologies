...
class EnhancedStoreComponent(V5EnhancedStore):
    ...
    def __init__(self, name: str, config: Dict[str, Any] = None):
        ...
        # Initialize database_config first to avoid AttributeError
        self.database_config = config.get('database', {})
        
        # Initialize V5EnhancedStore
    ...
    def _merge_configurations(self):
        ...
        # Extract database configuration from various sources
        config = getattr(self, 'config', {})
        
        # Merge database configuration from config
        if 'database_type' in config:
            self.database_config['database_type'] = config['database_type']
        
        if 'connection_url' in config:
            self.database_config['connection_url'] = config['connection_url']
        
        if 'table_name' in config:
            self.database_config['table_name'] = config['table_name']
