...
def test_main_component_generator_integration():
    """Test that the main component generator now imports V5EnhancedStore"""
    print("🧪 Testing Main Component Generator Integration...")
    
    try:
        # Simply test that we can check the imports in the generator
        ...
        # Read the source file to verify it imports V5EnhancedStore
        ...
        assert "from autocoder.components import V5EnhancedStore" in source_code, "Should import V5EnhancedStore"
        assert "class {class_name}(V5EnhancedStore):" in source_code, "Should use V5EnhancedStore in Store component generation"
        
        print("✅ Main component generator correctly imports and uses V5EnhancedStore")
        return True
...
def test_end_to_end_natural_language_pipeline():
    """Test that natural language input can generate V5 database systems"""
    print("🧪 Testing End-to-End Natural Language → V5 Database System Pipeline...")
    
    try:
        # This would be the ultimate test - natural language to V5 system
        # For now, test that the components can be imported and work together
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            comp_gen = V5EnhancedComponentGenerator(Path(temp_dir))
            
            # 2. System Generator
            sys_gen = V5EnhancedSystemScaffoldGenerator(Path(temp_dir))
            
            # 3. Database Orchestrator
            orchestrator = DatabaseValidationOrchestrator()
            
            # If we get here, all components are compatible
            print("✅ End-to-End pipeline components are compatible and working")
            return True
...
def main():
    ...
    print("🚀 Running Critical Phase 5 Database Integration Tests (SIMPLE)")
    print("=" * 80)
    ...
    tests = [
        ...
        test_end_to_end_natural_language_pipeline
    ]
