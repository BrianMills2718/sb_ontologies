#!/usr/bin/env python3
"""
phase8_test_api - Production Flask Application (Standalone)
...
- STANDALONE MODE: No autocoder framework dependencies required
"""
import os
import logging
import yaml
...
# V5.1 Component imports - STANDALONE MODE
...
def create_app():
    ...
    STANDALONE MODE - No external framework dependencies.
    """
    app = Flask(__name__)
    ...
    # Create and configure components - STANDALONE MODE
    ...
    # System info endpoint for debugging deployment issues
        ...
        try:
            ...
            system_info = {
                ...
                'environment_variables': {
                    ...
                    'API_SECRET_KEY': 'SET' if os.getenv('API_SECRET_KEY') else 'NOT_SET',
                    'FLASK_HOST': os.getenv('FLASK_HOST', 'not_set'),
                    'FLASK_PORT': os.getenv('FLASK_PORT', 'not_set')
                },
                'components': [
                    'test_api'
                ]
    ...
    def not_found(error):
        return jsonify({'error': 'Not found', 'mode': 'standalone'}), 404
    
    @app.errorhandler(500)
    def internal_error(error):
        logger.error(f"Internal server error: {error}")
        return jsonify({'error': 'Internal server error', 'mode': 'standalone'}), 500
    
    @app.errorhandler(503)
    def service_unavailable(error):
        return jsonify({'error': 'Service temporarily unavailable', 'mode': 'standalone'}), 503
    ...
    logger.info(f"phase8_test_api Flask application initialized for production (standalone mode)")
    return app
